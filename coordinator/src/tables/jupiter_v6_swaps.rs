        ...
        if program_id == &bs58::decode(PROGRAM_ID).into_vec().unwrap() {
            ...
            match &slice_u8[0..8] {
                idl_decoder::jupiter_v6::client::args::Route::DISCRIMINATOR => {
                    if let Ok(route) = idl_decoder::jupiter_v6::client::args::Route::deserialize(
                        &mut &slice_u8[8..],
                    ) {
                        log::debug!("TODO: handle route {:?}", route.route_plan);
                    } else {
                        log::warn!("Error deserializing Route");
                    }
                }
                _ => {
        ...
        let program_id = transaction
            ...
            .map(|message: &firehose_client::solana::Message| {
                if message.account_keys.len() > compiled_instruction.program_id_index as usize {
                    return message.account_keys[compiled_instruction.program_id_index as usize]
                        .as_slice();
                }
                let index =
            ...
            match &slice_u8[0..8] {
                idl_decoder::jupiter_v6::client::args::Route::DISCRIMINATOR => {
                    if let Ok(instruction) =
                        idl_decoder::jupiter_v6::client::args::Route::deserialize(
                            &mut &slice_u8[8..],
                        )
                    {
                        log::debug!("TODO: handle route {:?}", instruction.route_plan);
                    } else {
                        log::warn!("Error deserializing Route");
                    }
                }
            ...
            match block {
                Block::Solana { block } => {
                    ...
                    for transaction in block.transactions {
                        ...
                        // ------------- EVENTS -------------
                        let programs_selector = ProgramsSelector::new(&[PROGRAM_ID.to_string()]);
                        let log_contexts =
                            LogContext::parse_logs_basic(&meta.log_messages, &programs_selector);
                            ...
                            for data in context.data_logs.iter() {
                                if let Ok(decoded) = BASE64_STANDARD.decode(data) {
                                    ...
                                    match slice_discriminator {
                                            ...
                                            _other => {
                                                log::debug!("Unknown event: {:?}", _other);
                                            }
                                        }
                                } else {
                        ...
                        if let Some(ref meta) = transaction.meta {
                            for instruction in transaction
                                ...
                                .unwrap()
                            {
                                Self::process_compiled_instruction(meta, &transaction, instruction);
                            }
...
pub fn jupiter_v6_swaps_table(floor: i64, ceiling: Option<u64>) -> JupiterV6SwapsTableProvider {
    BlockBasedTableProvider::new_solana_mainnet_firehose(floor, ceiling, JupiterV6SwapsProcessor {})
}
